  ...
  try {
    const rows = await query(
      "SELECT course_topic.*, course.name as `course_name`, course_module.title as `course_module_title` FROM course_topic " +
        "LEFT JOIN course_module ON course_topic.id_module = course_module.id " +
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (aux.items && aux.items.length > 0) {
          ...
          for (let z = 0; z < aux.items.length; z++) {
            const insertedItem = await query("INSERT INTO course_topic SET ? ON DUPLICATE KEY UPDATE title = VALUES(title)", {
